import type { QTableProps } from 'quasar'
import { labelByEntity } from 'src/constants/labels'
import l from 'lodash'
import { sortedSupplyItemsByPlannedAt } from 'src/pages/components/DashBoard/SuppliesNotConfirmed/components/sortedSupplyItemsByPlannedAt'
import dayjs from 'dayjs'
import { formatToDDMMYY } from 'src/utils/dateFormatter'

export const columns = <QTableProps['columns']>[
  {
    name: 'supplier',
    align: 'left',
    label: l.get(labelByEntity, 'supply.supplier_id'),
    field: (row) => l.get(row, 'supplier.name'),
    format: (val) => `${val}`,
    sortable: true,
  },
  {
    name: 'planned_at',
    align: 'right',
    label: l.get(labelByEntity, 'supply_item.deliveries.planned_at'),
    field: (row) =>
      l.first(sortedSupplyItemsByPlannedAt(row?.supply_items))?.planned_at,
    format: (val) => formatToDDMMYY(val),
    sortable: true,
    sort: (a, b) => {
      const dateA = dayjs(a)
      const dateB = dayjs(b)
      return dateA.isBefore(dateB) ? -1 : dateA.isAfter(dateB) ? 1 : 0
    },
  },

  {
    name: 'link',
    label: '',
    required: true,
    field: 'link',
    align: 'right',
  },
]
