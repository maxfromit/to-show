...
import { ref } from 'vue'
import RelocationsNotConfirmedTable from 'src/pages/components/DashBoard/RelocationsNotConfirmed/RelocationsNotConfirmedTable.vue'
import RelocationsNotConfirmedFilterButtons from 'src/pages/components/DashBoard/RelocationsNotConfirmed/RelocationsNotConfirmedFilterButtons.vue'
...
<template>
  <VueThemer>
    ...
    >
      ...
      <template #header>
        Не подтверждено в перемещениях
        <q-tooltip theme="extra.delayWidth">
          Перемещения с позициями, в которых наступила или прошла
          {{
            l.lowerCase(l.get(labelByEntity, 'relocation_item.planned_in_at'))
          }}, и у которых есть неподтвержденные
          {{ l.lowerCase(getProductMoveName('relocation_item_id')) }}
        </q-tooltip>
      </template>
      <template #filter>
        <RelocationsNotConfirmedFilterButtons
